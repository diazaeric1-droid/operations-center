...
def render() -> None:
    ...
    if st.session_state.get("brief_view", "Unified list") == "Detailed panels":
        _detailed_panels(div, active, acked, anomalies, price, nri)
    else:
        _unified_list(events, active, net_view, price)

    _the_brief(token, price, nri, as_of, summary, anomalies, div, events)
...
def _detailed_panels(div, active, acked, anomalies, price: float,
                     nri: float) -> None:
    """The original three-panel layout (wells down / divergences, ranked anomalies,
    acknowledged) — unchanged, behind the view toggle."""
